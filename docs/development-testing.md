# Development fixtures and scanner testing

The current application requires migrations 001 through 009. Single registration creates a private draft, and bulk imports create pending approvals. Development previews never send or approve messages. Read [email delivery workflow](email-delivery-workflow.md) before testing email status.
...
Migration `005_shared_meal_scanner.sql` creates the dedicated `Meal Scanner` system identity, its default scanner/location and `Meal` type, and immutable browser request mappings. These are application defaults, not test employees or email recipients. Existing human accounts, catalog records, employee QRs, and meal history are preserved. The migration refuses reserved-code collisions rather than overwriting records.

Migration `008_employee_contact_details.sql` adds company and phone fields without filling or replacing existing employee data. Migration `009_master_qr_meal_allowances.sql` adds each master QR's visitor-group details and remaining meal allowance. The runtime database account needs `INSERT` and `UPDATE` on `master_qr_allocations` in addition to its existing privileges. Inspect the target's migration ledger before applying any pending versions; do not assume earlier versions are already applied. Database changes and privilege changes require separate approval, and neither launcher performs them.
...
| `TEST Admin Office` | Master QR with prefilled fictional visitor details and an allowance of 10 one-meal scans; expires when exhausted |
| `TEST Cafeteria`, `TEST Lunch`, `TEST Scanner A`, `TEST Scanner B` | Registered service settings for the scanning exercises |

All employee email addresses use `example.test`. Registration prepares local previews through the normal encrypted email queue. Inactive and revoked fixtures cancel their queued QR messages through the normal services; expired credentials cannot be previewed as valid emails. Use the development QR gallery to obtain these intentionally invalid test images. No sender or email delivery worker is invoked. A seed created before group allowances retains its original master QR on rerun; create a new fictional visitor-group QR through the admin application for the current scanner checks.
...
Browser storage is separate for each port. If the port 8001 scanner has no local marker, it uses its preserved signed browser cookie to find the latest stored request and recover its result before another meal can begin. A completed result remains visible for two seconds and then returns to the initial scanner screen. An uncertain result or failed recovery lookup blocks a new serving. Neither launcher kills an older server or any unrelated process.
...
2. Leave the same QR in front of the camera. Confirm the camera pauses, the accepted animation appears, and no second meal is recorded from repeated frames. After two seconds, confirm the initial scanner screen returns automatically. Deliberately start another scan of that employee QR and verify two distinct meals in the dashboard.
...
6. In the administrator's **Master QR** section, create a fictional group with Company Name, POC Name, Email, Phone Number, and five people. Use an `example.test` email. Download its single QR and scan it five deliberate times. Each scan must show **Accepted · 1 meal** only after commit, with no visitor form or approval prompt. Verify five distinct meals, the four saved contact fields, quantity one, `Meal Scanner` attribution, default `Meal` type, and UTC time in reports. The master list must show five used and zero remaining. A sixth scan must animate a rejection saying **QR has expired. Please contact the administrator.** and create no meal.
7. In the admin group form, try missing contact fields, malformed email/phone, zero, negative, and fractional people counts; no QR should be created. Revoke a separate allocated master QR before scanning and verify rejection. Confirm a different browser cannot recover the first browser's result or change its pending request. The public scanner must be unable to open employee management, the test QR gallery, or unrestricted report APIs.
...
13. With approved test data, create a group with one person. Submit distinct requests from two scanner browsers simultaneously. Exactly one may be approved; the other must report expiry. Retry the successful request using its original browser and UUID after exhaustion; it must return the original meal and leave the used count at one. This concurrency check requires the actual MySQL integration suite or an equivalent controlled manual exercise.
...
From the administrator application, open **Employees → Add employee** and verify the eight facility names in the department list. Existing active departments should still be selectable. Opening the form must not create department or employee records. With approved development data, enter company, phone, and an `example.test` email, select a facility department, and register a fictional employee; confirm the saved employee references the actual department record. Scan their QR and verify company, email, and phone in meal history. Update an older employee's previously empty company and phone fields and verify those values appear in their history. Repeat with another employee in the same department and verify no duplicate department is created. Keep deliberately inactive departments inactive.
...
2. Open the scanner application's root URL directly without staff sign-in. A production browser may need its one-time scanner activation code. Verify the scanner controls and result animations fit portrait and landscape screens. No visitor form, station selector, or meal selector should appear. Confirm each completed result resets after two seconds and that processing or connection errors do not reset an uncertain serving.
...
5. Repeat employee reuse, invalid QR, master allowance exhaustion, and interrupted-connection exercises from the laptop checklist. Verify the browser camera indicator stops after leaving the page, closing the tab, or backgrounding where supported. With the phone's reduced-motion setting enabled, verify the result remains clear without relying on movement.
