...
- Scanner-device activation using a private server-side secret and a signed HttpOnly browser cookie. The scanner has no staff login. A visitor's or employee's QR alone does not activate a scanner device.
...
4. Generate a private scanner activation code, store it outside Git with mode `0600`, and add it to the scanner project's Production `SCANNER_ACTIVATION_SECRET`. Keep `SCAN_APP_ENABLED=false` until that secret is configured. New devices enter the code once and can then move between office Wi-Fi and mobile data.
...
Check laptop and phone cameras using the actual HTTPS scanner origin. Activate a new browser, switch between Wi-Fi and mobile data, and confirm that its scanner identity remains unchanged. Confirm rejection before activation, session isolation, no administrator APIs on the scanner project, and no private content in CDN caches. Only then provide the scanner activation code to approved device operators. Unit tests and packaging checks do not replace these hosted integration and device checks.
