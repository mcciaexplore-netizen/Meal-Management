...
Employee registration stores the code, name, required email, department, active status, and optional selfie reference. Registration creates the employee, random QR, encrypted single-email draft, and audit entries in one transaction. The administrator clicks Send email in employee details to deliver that draft; registration itself sends nothing. Bulk CSV import creates up to 100 employees atomically and holds their emails for explicit approval in the email queue. See [the email workflow](docs/email-delivery-workflow.md). Each employee has at most one unrevoked QR, and their QR can record any number of deliberate meals over time.
...
QRs contain random bearer tokens, never names or employee codes. MySQL stores a SHA-256 digest for lookup and an encrypted recoverable token for administrator-only display and resending. Resending prepares the same active QR for an explicit Send email action and reuses an existing single draft; it does not generate another credential. Issuance refuses an existing unrevoked QR atomically. Replacement revokes the old credential and prepares a single-email draft for the replacement in the same transaction. Revocation removes recoverable credential material and cancels pending delivery. Expired credentials must be replaced or revoked before another current credential is issued.

Deactivation blocks new employee meals and cancels pending emails without deleting employee or meal history. An administrator can reactivate an employee. Name and email changes preserve the employee identifier; changing email cancels stale delivery and prepares the current usable QR as a new draft for the new address.
...
Gmail delivery uses an App Password over verified TLS. After migration 006 and configuration approval, `EMAIL_SEND_ENABLED=true` permits explicit single sends and approved bulk processing from the dashboard. `EMAIL_AUTO_SEND_ENABLED` can stay false. An optional background sender processes only approved bulk or legacy entries; it never selects single drafts or pending approvals. Approval attribution and existing delivery claims are durable. The scanner continues independently when administration is stopped.

The sender creates a durable claim before each provider call and records acceptance only after a successful database commit. Failed or uncertain sends require review and are never automatically retransmitted. Global sender or authentication failures halt automatic processing until configuration is corrected and the admin server is restarted. Real sending and automatic processing remain disabled by default. The guarded `send-email` CLI command can process an already-approved entry but cannot bypass approval. The administrator-only browser Send email action is scoped to one employee and one durable email ID. See [Gmail configuration and sending instructions](docs/gmail-setup.md).
